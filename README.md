...
- 📖 **书架** — 创建 / 打开作品，搜索与字数统计  
- ✍️ **写作页** — 章节侧栏、自动保存、全屏编辑  
- 🤖 **AI** — 续写 / 润色 / 扩写 / 自定义指令（OpenAI 兼容流式）  
- ⚙️ **API 配置** — 官方 / 自定义 endpoint + 模型  
- 💾 **备份** — JSON 导出 / 导入（兼容原 `novels` 键）  

所有数据保存在设备本地（IndexedDB + localStorage），**不上传服务器**。AI 请求走你自行配置的 API。

> 旧桌面风格 Element Plus 页面已不再作为 App 主界面；核心存储键与 API 服务仍复用，历史作品可直接打开。
